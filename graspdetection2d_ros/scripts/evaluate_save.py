...
if __name__ == '__main__':
    ...
    grasp_files = glob.glob(os.path.join(args.dataset_path, '*', 'pcd*cpos.txt'))  #Added
    grasp_files.sort()                                                             #Added
    file_names=[f.replace('cpos.txt', '') for f in grasp_files]                    #Added
    file_names=[f.replace(args.dataset_path, '') for f in file_names]              #Added
    file_names=[f.replace('/', '') for f in file_names]                            #Added
    file_names=file_names[int(len(file_names)*args.ds_rotate):] + file_names[:int(len(file_names)*args.ds_rotate)]  #Added
    ...
    if args.ds_shuffle:
        ...
        np.random.shuffle(file_names)                                              #Added
    val_indices = indices[split:]
    
    #val_indices = [15,16,17,18,149,150,151,152,303,304,305,307,568,566,567,568]

    # val_sampler = torch.utils.data.sampler.SubsetRandomSampler(val_indices)
    val_sampler = torch.utils.data.sampler.SequentialSampler(val_indices)
    print(val_indices)
    # print(val_sampler.indices)
    #print(file_names[:11])

    file_names=file_names[split:]                                                  #Added
